...
namespace GameReviews.Controllers {
    ...
    public class ManageController : Controller {
        ...
        public ActionResult ChangePicture(ChangePictureViewModel model) {
            var user = db.Users.ToList().Find(u => u.Id == User.Identity.GetUserId());
            if (user == null) return HttpNotFound();
            user.PictureURL = model.PictureURL;
            var comments = db.Comments.Where(c => c.UserName == user.UserName).ToList();
            foreach(var comment in comments) {
                comment.UserPhotoURL = user.PictureURL;
            }
            db.SaveChanges();
